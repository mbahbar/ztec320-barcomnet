...
def onu_wan_service_edit(onu_id, svc_idx):
    ...
    try:
            ...
            if err:
                ...
                # Handle 63869 "Record already exists" — force delete and retry once
                    ...
                    if cmd.startswith('pppoe '):
                        idx = cmd.split()[1]
                        tc._send_command(tn, f'no wan {idx}', timeout=10)
        ...
        # Must remove WAN binding FIRST, then pppoe/wan-ip, then service record
        # NOTE: V2.1.0 uses 'no wan N', NOT 'no wan N service' (Invalid command key word)
        tc._send_command(tn, f'no wan {svc_idx}', timeout=10)
        ...
        if status == 'enable':
            if mode == 'Bridge / ONU Webpage':
                ...
                vlan_profile = data.get('vlan_profile') or f'PPPoE-{vlan}'
                if username:
                    sc(f'wan-ip {svc_idx} mode pppoe username {username} password {password} vlan-profile {vlan_profile} host {svc_idx}')
                sc(f'security-mgmt {svc_idx} state enable mode forward')
...
def scan_unconfigured():
    ...
    def match_onu_type(model, sn=''):
        ...
        if '0425' in ml:
            for preferred in ['G-0425G-H', 'NOKIA-G0425G', 'NOKIA']:
                if preferred in reg_types: return preferred
        if 'ALCL' in sn_up or 'NOKIA' in sn_up or 'NOKIA' in ml or 'G-' in ml:
            for preferred in ['NOKIA', 'G-0425G-H', 'NOKIA-G240W', 'NOKIA-G010G', 'HUAWEI', 'ZXHN-F660']:
                if preferred in reg_types: return preferred
